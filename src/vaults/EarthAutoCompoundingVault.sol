...
abstract contract EarthAutoCompoundingVault is
    ERC4626,
    Ownable,
    ReentrancyGuard,
    Initializable
{
    ...
    function maxDeposit(
        ...
        uint256 maxFromUserTvlCap = userCap > 0
            ? userTvlCap[receiver] - previewMint(balanceOf(receiver))
            : type(uint256).max;
        return
            maxFromTotalTvlCap < maxFromUserTvlCap
                ? maxFromTotalTvlCap
                : maxFromUserTvlCap;
    }
    ...
    function maxMint(
        ...
        if (strategy.paused()) return 0;
        uint256 maxFromTotalTvlCap = this.convertToShares(
            totalTvlCap - this.totalAssets()
        );
        uint256 userCap = userTvlCap[receiver];
        uint256 maxFromUserTvlCap = userCap > 0
            ? convertToShares(userTvlCap[receiver]) - balanceOf(receiver)
            : type(uint256).max;
        return
            maxFromTotalTvlCap < maxFromUserTvlCap
                ? maxFromTotalTvlCap
    ...
    function proposeStrat(address _implementation) public {
        require(
            owner() == _msgSender() || strategy.manager() == _msgSender(),
            "!(owner || manager)"
        );
        require(
            address(this) == IStrategy(_implementation).vault(),
            "!proposal"
        ); //Stratey also holds the address of the vault hence equality should hold
    ...
    function upgradeStrat() public {
        ...
        require(
            ...
            "!delay"
        ); //Approval delay should have been passed since proposal time
